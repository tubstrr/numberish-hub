import { ascii, asciiReverse, codex, codexReverse } from '@/composables/codex.js';
...
export const decrypt = (method, value) => {
 ...
 if (method === 'RJ') {
  ...
  return decodedArray.map((letter) => codexReverse[letter] || letter).join('');
 }

 if (method === 'ASCII') {
  const decodedArray = value.match(/\d{2}|.{2}/g) ?? [];
  return decodedArray.map((letter) => asciiReverse[letter] || letter).join('');
 }
